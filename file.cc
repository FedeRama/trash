...
PurePursuitNode::PurePursuitNode()
{
  ...
  if(launch_mode == MODE_FRENET)
  {
    ...
    opt_path_splines[PATH_LOCAL] = nullptr;
...
void PurePursuitNode::local_path_callback(const nav_msgs::msg::Path::SharedPtr path)
{
	...
	opt_path_splines[PATH_LOCAL] = new Spline2D(a,b);
...
void PurePursuitNode::main_loop()
{
  ...
  //{
    ...
    Spline2D* cur_path_spline;

    if(launch_mode == MODE_FRENET)
    {
      if (opt_path_splines[PATH_LOCAL] == nullptr)
      {
        //std::cout << "[WARNING] The chosen path does not exist yet!" << std::endl;
        return;
      }
       
      cur_path_spline = opt_path_splines[PATH_LOCAL];
 
    }
    if(launch_mode == MODE_HEAD2HEAD)
    {
      cur_path_spline = opt_path_splines[cur_path];     
    }
